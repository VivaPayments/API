<?php
  class osC_Payment_hellaspay extends osC_Payment {
    var $_title,
        $_code = 'hellaspay',
        $_status = false,
        $_sort_order,
    ...
    function osC_Payment_hellaspay() {
	  ...
	  $this->_lang_code = $osC_Language->get('payment_hellaspay_language_code');
      $this->_status = (MODULE_PAYMENT_HELLASPAY_STATUS == '1') ? true : false;
      $this->_sort_order = MODULE_PAYMENT_HELLASPAY_SORT_ORDER;
		
		switch (MODULE_PAYMENT_HELLASPAY_MODE) {
        case 'Live':
      ...
      if ($this->_status === true) {
        ...
        if ((int)MODULE_PAYMENT_HELLASPAY_ZONE > 0) {
          $check_flag = false;

          $Qcheck = $osC_Database->query('select zone_id from :table_zones_to_geo_zones where geo_zone_id = :geo_zone_id and zone_country_id = :zone_country_id order by zone_id');
          $Qcheck->bindTable(':table_zones_to_geo_zones', TABLE_ZONES_TO_GEO_ZONES);
          $Qcheck->bindInt(':geo_zone_id', MODULE_PAYMENT_HELLASPAY_ZONE);
    ...
    function process_button() {
      ...
      if (MODULE_PAYMENT_HELLASPAY_CURRENCY == 'Selected Currency') {
        $currency = $osC_Currencies->getCode();
      } else {
        $currency = MODULE_PAYMENT_HELLASPAY_CURRENCY;
      }
	  
	  $hellaspay_amount = round($osC_Currencies->formatRaw($osC_ShoppingCart->getTotal(), $currency), 2) * 100;
	...
	if(MODULE_PAYMENT_HELLASPAY_MODE=='Testing'){
	...
	$response = curl_exec($curl);
	  ...
	  $Stransaction = $osC_Database->query('insert into :hellaspay_data (OrderCode, ErrorCode, ErrorText, Timestamp, ref, total_cost, currency, order_state, sessionid) values (:OrderCode, :ErrorCode, :ErrorText, now(), :ref, :total, :currency, :state, :sessid)');
            
			$Stransaction->bindTable(':hellaspay_data', 'hellaspay_data');
			...
			$Stransaction->bindValue(':total', $hellaspay_amount);
    ...
    function callback() {
	  ...
	  if(preg_match("/fail/i", $_SERVER['REQUEST_URI'])) {
 	  ...
 	  $check_query = "select ref, sessionid from hellaspay_data where OrderCode='".addslashes($_GET['s'])."'";
		...
		$update_query = "update hellaspay_data set order_state='F' where OrderCode='".addslashes($_GET['s'])."'";
		$uQuery = $osC_Database->query($update_query);
		$uRecordset = $uQuery->execute();

		$error = $osC_Language->get('payment_hellaspay_error_message');
	  ...
	  if(preg_match("/success/i", $_SERVER['REQUEST_URI'])) {
 	  ...
 	  $check_query = "select ref, sessionid from hellaspay_data where OrderCode='".addslashes($_GET['s'])."'";
		...
		$update_query = "update hellaspay_data set order_state='P' where OrderCode='".addslashes($_GET['s'])."'";
		$uQuery = $osC_Database->query($update_query);
		$uRecordset = $uQuery->execute();
