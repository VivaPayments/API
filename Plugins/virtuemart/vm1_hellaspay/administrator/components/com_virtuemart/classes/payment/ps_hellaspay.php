...
/**
*@Viva Payments Payment Gateway
* @version : ps_hellaspay.php 2013-01-11 Created By Viva Wallet
*/

class ps_hellaspay {
    var $classname = "ps_hellaspay";
    var $payment_code = "Viva Payments";
    /**
...
function CopyAndPaste( from, to )
{
    ...
    <table>
        ...
        <td width="200"><strong>Merchant ID</strong></td>
            <td width="305"><input type="text" name="HELLASPAY_ID" class="inputbox" value="<?php  echo HELLASPAY_ID; ?>" /></td>
            <td width="300">The Merchant ID for Viva Payments. </td>
			</tr>
         <tr>
        <td width="200"><strong>API Key</strong></td>
            <td width="305"><input type="text" name="HELLASPAY_PASS" class="inputbox" value="<?php  echo HELLASPAY_PASS; ?>" /></td>
            <td width="300"></td>
			</tr> 
         <tr>
        <td width="200"><strong>Source</strong></td>
            <td width="305"><input type="text" name="HELLASPAY_SOURCE" class="inputbox" value="<?php  echo HELLASPAY_SOURCE; ?>" /></td>
            <td width="300">Viva Payments Source code. </td>
			</tr>   
			<tr>
		      <td><strong>Form Viva Payments</strong></td>
            <td colspan="2">
<textarea name="HELLASPAY_FORM" cols="80" rows="15" readonly="readonly" STYLE="display:none;">
<?php echo "<?php\n"; ?>

$tax_total = $db->f("order_tax") + $db->f("order_shipping_tax");
$discount_total = $db->f("coupon_discount") + $db->f("order_discount");

$hellaspay_amount = number_format($db->f("order_total"), 2, '.', '') * 100;
...
$auth = $_SESSION['auth'];
 // Get user billing information
	...
	$poststring['SourceCode'] = HELLASPAY_SOURCE;
    ...
    $postargs = 'Amount='.urlencode($poststring['Amount']).'&RequestLang='.urlencode($poststring['RequestLang']).'&Email='.urlencode($poststring['Email']).'&MaxInstallments='.urlencode($Installments).'&MerchantTrns='.urlencode($poststring['MerchantTrns']).'&SourceCode='.urlencode($poststring['SourceCode']).'&PaymentTimeOut=300';
		...
		curl_setopt($curl, CURLOPT_USERPWD, HELLASPAY_ID.':'.html_entity_decode(HELLASPAY_PASS));
...
echo '<input type="image" name="submit" src="'.SECUREURL .'components/com_virtuemart/shop_image/ps_image/hellaspay.jpg" border="0"/>';
echo '</form>';
?>
</textarea>
<BUTTON onClick="CopyAndPaste('HELLASPAY_FORM', 'payment_extrainfo')">
  ...
  /**
   ...
   function write_configuration( &$d ) {
      ...
      $my_config_array = array(
				  "HELLASPAY_ID" => $d['HELLASPAY_ID'],
		  		  "HELLASPAY_PASS" => $d['HELLASPAY_PASS'],
				  "HELLASPAY_SOURCE" => $d['HELLASPAY_SOURCE']
				  );
      $config = "<?php\n";
