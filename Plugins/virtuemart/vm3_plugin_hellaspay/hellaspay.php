...
//EB AFTER ORDER PAGE
	function _getHtmlPaymentResponse($msg, $is_success=true, $order_id=null, $order_nr=null, $amount=null, $txid=null, $period=null, $date=null, $paymethodid=null) {
		...
		$datenew = explode(" ", $date);
			...
			$html .= $this->getHtmlRow('HELLASPAY_ORDER_NUMBER', $order_id .'/'.$order_nr, 'style="width: 90px;" class="key"');
			$html .= $this->getHtmlRow('HELLASPAY_AMOUNT', $amount . ' ' . $currency_code_3, 'style="width: 90px;" class="key"');
			//VM3 added
            $html .= $this->getHtmlRow('HELLASPAY_TXID', $txid, 'style="width: 90px;" class="key"');
			if(isset($period) && $period > 1){
			$html .= $this->getHtmlRow('HELLASPAY_PERIOD', $period, 'style="width: 90px;" class="key"');
			}
			$html .= $this->getHtmlRow('HELLASPAY_DATE', $datenew[0], 'style="width: 90px;" class="key"');
...
//EB HANDLE RESULT
	function managePaymentResponse($virtuemart_order_id, $ref, $resp, $new_status, $return_context=NULL, $order_number, $period=NULL) {
		...
		if ($resp=='SUCCESS'){
		$order['comments'] = vmText::sprintf('VMPAYMENT_HELLASPAY_PAYMENT_STATUS_CONFIRMED', $order_number) . $ref . $periodmsg;
		} else {
		$order['comments'] = vmText::sprintf('VMPAYMENT_HELLASPAY_PAYMENT_STATUS_CANCELED', $order_number) . $ref;
		}
...
//EB PAYMENT NOTIFICATION VALIDATION - CONFIRMATION
	function plgVmOnPaymentNotification() {
		...
		$hellaspay_data = vRequest::getPost();
...
//EB DISPLAY PAYMENT OPTIONS ON CHECKOUT HELPER	
	function listInstalments($list_name, $selected=false, $class='', $method, $cart) {
		...
		$split_instal_hellaspay = explode(',', $instalmentoptions);
		$c = count ($split_instal_hellaspay);
		$options[] = JHTML::_('select.option', '1', vmText::_('VMPAYMENT_HELLASPAY_INSTALMENTS_NO'));
		...
		$options[] = JHTML::_('select.option', $instal_term, $instal_term . vmText::_('VMPAYMENT_HELLASPAY_INSTALMENTS_CHECKOUT_OPTION'));
		}
		}
...
//EB DISPLAY SAVED PAYMENT INFO
	...
	$addpricenote = " (+ " . number_format($this->addprice, 2, ',', '')." ".$currency_code_3.")";
		...
		if(isset($activeMethod->hellaspay_instalments) && $activeMethod->hellaspay_instalments!=''){
		if(count($this->payments) < 2){
		if(isset($_GET['task'])){
		$creditCardInfos .= '';
		} else {
		$creditCardInfos .= '&nbsp;<a href="' . JURI::base() .'index.php?option=com_virtuemart&view=cart&task=editpayment" "target="_self">'. vmText::_('VMPAYMENT_HELLASPAY_OPTIONS') . '</a>' . '<input type="hidden" name="vivinstalmentsamount" value="'.$this->_vivinstalmentsamount.'"><input type="hidden" name="vivinstalments" value="'.$this->_vivinstalments.'">';
		}
		}
		}
