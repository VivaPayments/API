...
//
// $Id: hellaspay.php 11696 2013-01-11 09:30:02Z vivawallet $
//
...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	$hpoid = db_get_row("SELECT hp_oid FROM hellaspaydata WHERE hp_code = '".$hpordercode."'");
	$order_id = (strpos($hpoid['hp_oid'], '_')) ? substr($hpoid['hp_oid'], 0, strpos($hpoid['hp_oid'], '_')) : $hpoid['hp_oid'];
		...
		if ($order_info['payment_info']['OrderCode'] == $_GET['s']) {
			...
			$pp_response['reason_text'] = fn_get_lang_var('hellaspay_transaction_fail');
		}

		if (fn_check_payment_script('hellaspay.php', $order_id)) {
	...
	if (!empty($payment_info['installments']) && $payment_info['installments'] > 1) {
		$hellaspay_period = $payment_info['installments'];
	} else {
		$hellaspay_period = '1';
	}
	...
	if($hellaspay_period > 1){
	$poststring['MaxInstallments'] = $hellaspay_period;
	} else {
	...
	curl_close($curl);
		...
		db_query("INSERT INTO hellaspaydata (hp_oid, hp_code) VALUES ('".$cart_order_id."', '".$pp_response['OrderCode']."')");
		fn_update_order_payment_info($order_id, $pp_response);	
		$_SESSION['stored_hellaspay_orderid'] = $order_id;
...
$msg = str_replace('[processor]', 'Viva Payments server', $msg);
echo <<<EOT
	</form>
