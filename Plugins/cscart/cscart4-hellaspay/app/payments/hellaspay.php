...
//
// $Id: hellaspay.php 11696 2013-06-27 09:30:02Z vivawallet $
//
...
if (defined('PAYMENT_NOTIFICATION')) {
	...
	$order_id = (strpos($hpoid['hp_oid'], '_')) ? substr($hpoid['hp_oid'], 0, strpos($hpoid['hp_oid'], '_')) : $hpoid['hp_oid'];
		...
		if ($order_info['payment_info']['OrderCode'] == $_GET['s']) {
			...
			$pp_response['reason_text'] = __('hellaspay_transaction_fail');
		}
	...
	foreach ($currencies as $k => $v) {
		if ($k == $currency_code) {
			...
			$hellaspay_total = number_format((float)$amount, 2, '.', '');	
		}
	}
	
	if(!isset($hellaspay_total) || $hellaspay_total < 1){
