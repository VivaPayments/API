...
<div class="form-field">
	<label for="cc_instalments">{$lang.vivawallet_instalments}:</label>
	<select id="cc_instalments" name="payment_info[installments]">
		<option value="0" {if $cart.payment_info.installments == 1}selected="selected"{/if}>{$lang.vivawallet_no_instalments}</option>
		...
		{if !($payment_method.params.installments && $smarty.section.foo.index>$payment_method.params.installments)}
			<option value="{$smarty.section.foo.index}" {if $cart.payment_method.installments == $smarty.section.foo.index}selected="selected"{/if}>{$smarty.section.foo.index} {$lang.vivawallet_instalments}</option>
		{/if}
		{/section}
	</select>
</div>
