...
class Mage_Vivawallet_HpayController extends Mage_Core_Controller_Front_Action
{
	...
	public function getVivawallet()
    {
        return Mage::getSingleton('vivawallet/hpay');
    }
	
	public function setVivawalletResponse($response)
    {
    	if (count($response)) {
            $this->_vivawalletResponse = $response;
        } else {
			$this->_vivawalletResponse = null;
		}
        return $this;
    }
    ...
    public function redirectAction()
    {
		...
		$this->getResponse()->setBody($this->getLayout()->createBlock('vivawallet/hpay_redirect')->toHtml());
        $session->unsQuoteId();
	...
	public function HpokAction()
    {
		...
		$this->setVivawalletResponse($this->getRequest()->getPost());
		...
		//} elseif($this->getVivawallet()->getConfigData('order_status') !='pending') {
		} else {
		$newstate = $newstatus;
