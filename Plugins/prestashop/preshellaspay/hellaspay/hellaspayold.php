...
class hellaspay extends PaymentModule
{
	...
	$this->HellaspayUrl = 'https://www.vivapayments.com/web/newtransaction.aspx';
		...
		$instal_logic = Configuration::get('HELLASPAY_INSTAL');
		if(isset($instal_logic) && $instal_logic !=''){
		$split_instal_hellaspay = explode(',', $instal_logic);
		...
		$instal_hellaspay = '';
		$instal_template = '';
		$instal_hellaspay .= $this->l('Instalments: ') . '<select '.$selectdisable.' name="wperiod" onChange="this.form.submit();">'."\n";
		
		$instal_hellaspay .= '<option value="">'.$this->l('Make a selection').'</option>'."\n";
		$instal_hellaspay .= '<option value="0"'. (isset($_REQUEST['wperiod']) && $_REQUEST['wperiod']=='0' ? ' selected="selected"' : '') . '>'.$this->l('No Instalments').'</option>'."\n";
		...
		$instal_hellaspay .= '<option value="'.$instal_term.'"'. (isset($_REQUEST['wperiod']) && $_REQUEST['wperiod']==$instal_term ? ' selected="selected"' : '') . '>'. $instal_term . $this->l(' instalments').'</option>'."\n"; 
		}
		}
		$instal_hellaspay .= '</select>'.$this->l(' wait until the confirmation button appears.').'<br><br>'."\n";
	
		
		} else {
		$instal_template = 'false';
		$instal_hellaspay = '<input type="hidden" name="wperiod" value="" />';
		}
		...
		'Ref' 				=> $OrderCode,
		'WbInstalLogic' 	=> $instal_hellaspay,
	...
	$MerchantID =  Configuration::get('HELLASPAY_MERCHANTID');
	$Password =   Configuration::get('HELLASPAY_MERCHANTPASS');
	...
	$this->HellaspayUrl = 'https://www.vivapayments.com/web/newtransaction.aspx';
		...
		if($instal_template=='true'){
			...
			if($showDropdown!='1'){
			return array(
					'cta_text' => $this->l('Pay with Viva') . '<form id="hellaspay_confirmation_form" name="hellaspay_confirmation" data-ajax="false" action="'.Tools::getHttpHost(true) . $_SERVER['REQUEST_URI'].'" method="post">'.$instal_hellaspay.'</form>',
				...
				if(isset($modInstallments) && $modInstallments > 0){
				 ...
				 $instal_logo = Media::getMediaPath(_PS_MODULE_DIR_.$this->name.'/hellaspay_eu.gif');
				} else {
				 $instal_title = '';
				 $instal_logo = Media::getMediaPath(_PS_MODULE_DIR_.$this->name.'/hellaspay.gif');
				}
